...
3. **`db.py`'deki PostGIS/RPC notunu çöz**: Supabase'in PostgREST'i WKT
   string'ini doğrudan `geometry` kolonuna yazmayabilir. `db.py` içinde
   önerilen SQL fonksiyonunu (RPC) Supabase'de oluşturup
   `insert_ndvi_measurements()` fonksiyonunu buna göre güncellemen gerekecek.
