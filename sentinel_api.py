...
def parse_statistics_response(raw_response: dict) -> list[dict]:
    ...
    NOT: Sentinel Hub Statistical API'nin tam yanıt şeması periyodik
    olarak küçük farklılıklar gösterebiliyor (ör. "data" listesi
    içinde "outputs" -> "ndvi" -> "bands" -> "B0" -> "stats" yolu).
    Gerçek bir yanıt örneğiyle bu fonksiyonun alan yollarını
    doğrulaman gerekecek — bkz. aşağıdaki TODO.
    """
    records = []

    # TODO: Bu blok, gerçek bir API yanıtıyla test edilip
    # doğrulanmadan güvenilir kabul edilmemeli. Sentinel Hub'ın
    # güncel dokümantasyonundaki örnek yanıt yapısına göre
    # alan isimleri (data / outputs / ndvi / bands / stats vb.)
    # burada teyit edilmeli.
    for interval_data in raw_response.get("data", []):
        ...
        datamask_stats = outputs.get("dataMask", {}).get("bands", {}).get("B0", {}).get("stats", {})

        # dataMask'ın mean değeri kabaca "bulutsuz/geçerli piksel oranı" gibi okunabilir;
        # cloud_cover_pct kabaca (1 - o oran) * 100 olarak hesaplanabilir.
        valid_fraction = datamask_stats.get("mean")
        cloud_cover_pct = (
            round((1 - valid_fraction) * 100, 2) if valid_fraction is not None else None
        )
