...
-- ÖNEMLİ: supabase_add_unique_constraint.sql'i de çalıştırdıysan (line_id,
-- measurement_date) üzerinde unique constraint var demektir; bu fonksiyon
-- ON CONFLICT ile upsert yapar (aynı hat+tarih tekrar gelirse günceller,
-- duplicate satır oluşturmaz). Constraint'i henüz eklemediysen bu fonksiyon
-- yine de normal INSERT gibi çalışır (ON CONFLICT tetiklenmez).
...
BEGIN
    ...
    ON CONFLICT (line_id, measurement_date) DO UPDATE SET
