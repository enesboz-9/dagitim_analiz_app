...
def main():
    ...
    parser.add_argument("--all", action="store_true",
                         help="is_monitored bayrağını YOK SAY, power_lines tablosundaki "
                              "TÜM hatları işle. DİKKAT: Türkiye genelinde binlerce hat "
                              "import edildiyse bu Sentinel Hub kredisini/kotasını çok "
                              "hızlı tüketir. Zamanlanmış çalıştırmalarda KULLANMA.")
    parser.add_argument("--province", type=str, default=None,
                         help="Sadece belirtilen ile ait hatları işle (is_monitored'dan "
                              "bağımsız) — manuel/on-demand tek il sorgusu için.")
    parser.add_argument("--external-ids", type=str, default=None,
                         help="Virgülle ayrılmış external_id listesi — sadece bu hatları "
                              "işle (is_monitored'dan bağımsız), manuel/on-demand sorgu için. "
                              "Örn: --external-ids way/12345,way/67890")
    ...
    external_ids = [x.strip() for x in args.external_ids.split(",")] if args.external_ids else None
    # --province ya da --external-ids verildiyse bu manuel/hedefli bir çalıştırmadır;
    # is_monitored bayrağından bağımsız olarak o hedefleri işle. Aksi halde --all
    # verilmedikçe varsayılan (kredi-dostu) davranış: sadece is_monitored=true hatlar.
    only_monitored = not (args.all or args.province or external_ids)
    if args.all:
        logger.warning("--all verildi: is_monitored bayrağı yok sayılıyor, TÜM hatlar "
                        "işlenecek. Bu Sentinel Hub kredisini hızlı tüketebilir.")
    lines = get_active_power_lines(only_monitored=only_monitored,
